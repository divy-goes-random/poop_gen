...
fn main() {
    ...
    let cnf: config::Config = match parse_config(config_path) {
        Ok(c) => c,
        Err(err) => {
    ...
    if let Some(ref includes) = cnf.include {
        for incl in includes {
            ...
            let conf: config::Config = match parse_config(path) {
                Ok(c) => c,
                Err(e) => {
...
fn process_config(cnf: config::Config) {
    let order: Vec<String> = match cnf.order {
        Some(order) => order,
        None => {
            vec!["mkdir".to_string(),
                 "gitignore".to_string(),
    ...
    for ord in order {
        match &*ord {
            "mkdir" => mkdir(&cnf.mkdir),
            "gitignore" => gitignore(&cnf.gitignore),
            "touch" => touch(&cnf.touch),
            "exec" => exec_list(&cnf.exec),
...
fn parse_config(file_name: String) -> Result<config::Config> {
